...
void LedC_Task(void *parameter) {
  ...
  for (;;) {
    ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0, amplitude);
    ledc_update_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0);
    amplitude += amplitudeStep;
    if (amplitude <= 0 || amplitude >= 8191) {
...
void SigmaDelta_Task(void *parameter) {
  ...
  sigmadelta_cfg.sigmadelta_gpio = (gpio_num_t)5;
  sigmadelta_cfg.sigmadelta_prescale = 80;
  ...
  for (;;) {
    static uint8_t i = 0;
    sigmadelta_set_duty(SIGMADELTA_CHANNEL_0, i++);
    delay(1);  // Add a small delay to prevent watchdog trigger
    // Alternatively, use vTaskDelay(1 / portTICK_PERIOD_MS);
  }
  vTaskDelete(NULL);
}
